...
def push_to_csv(fname,full_text):
    print "Appending to CSV"
    pdf_url = "https://s3.amazonaws.com/hackathonwie/"+fname[:-4]+".mp4"

    row = [[pdf_url, full_text]]
...
for line in fr:
    ...
    for i in range(0,len(text_sentences)):
        tts = gTTS(text=text_sentences[i], lang='en', slow=False, lang_check=False)
        try:
    ...
    if number_of_images > 0:
        for f in s_file_names:
            ...
            name_txt_clip = TextClip(format_text(' '.join([word[:1].upper()+word[1:] for word in f.split('/')[0].split('_')])),font='Arial-Bold',fontsize=200,color='blue',bg_color='black',stroke_width=30).set_position('top').set_duration(audio_clip.duration/number_of_images).resize(height=30)
